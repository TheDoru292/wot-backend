...
exports.checkId = (req, res, next) => {
  if (ObjectId.isValid(req.params.tweetId)) {
    ...
    return res
      .status(400)
      .json({ success: false, status: "Invalid ID provided." });
  }
};

exports.checkSameUserConversation = (req, res, next) => {
  if (req.user._id == req.userId) {
    return res.status(400).json({
      success: false,
      status: "You're trying to start a conversation with yourself!",
    });
  }
...
exports.checkConversationExists = (req, res, next) => {
  conversation.findOne(
    { users: { $all: [req.user._id, req.userId] } },
    (err, conversation) => {
      if (err) {
        const Error = new ErrorHandler(err, 500);
        return res.status(Error.errCode).json(Error.err);
      }

      if (conversation) {
        return res.status(400).json({
          success: false,
          status: "Conversation with user already exists!",
        });
      }
...
exports.checkConversationParticipant = (req, res, next) => {
  conversation.findOne(
    { _id: req.params.conversationId, user: { $in: req.user._id } },
    (err, conversation) => {
      if (err) {
        const Error = new ErrorHandler(err, 500);
        return res.status(Error.errCode).json(Error.err);
